...
const fetch = (path: string, init?: RequestInit) => app.request(new Request(`http://local${path}`, init));
...
Deno.test('base root guest redirects to home-agent', async () => {
  ...
  if (!r.headers.get('Location')?.endsWith('/home-agent')) throw new Error('expected redirect to home-agent');
});
...
Deno.test('app host adds face and renders', async () => {
  const r1 = await app.request(new Request('http://local/other-agent', { headers: { host: 'your-app.dreamcatcher.ai' } }));
  ...
  if (!html.includes('Agent:') || !html.includes('Face:')) throw new Error('page did not render agent view');
});
