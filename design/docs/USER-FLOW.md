...
sequenceDiagram
  ...
  participant F as App Host
  participant A as Default Agent (multi-face)
  U->>F: GET / (no ?face)
  F->>A: create_face()
  A-->>F: {face_id}
  F-->>U: 302 Location /?face={face_id}
  U->>F: GET /?face={face_id}
  F->>A: attach(face_id)
...
journey
  ...
  section First Login (New User)
    ...
    Create Customer App (slug, region): 2: Artifact MCP
    Create base agent machine (1 shared vCPU / 1GB) in Customer App: 3: Artifact MCP
...
- **Face URL:** `/{agent_path}/?face={face_id}`.
...
sequenceDiagram
  ...
  U->>F: GET /concierge (no session)
  F->>R: create_face(agent=concierge)
  R-->>F: {face}
  F-->>U: 302 Location /concierge?face={face}
  U->>F: GET /concierge?face={face}
  F->>C: attach(face) via TTYD
...
sequenceDiagram
  ...
  participant R as Registry/Face Store
  U->>F: GET /a/{user} (no session)
  F->>R: ensure_base_agent(user)
  R-->>F: {endpoint}
  F->>R: create_face(agent=base@{user})
  R-->>F: {face}
  F-->>U: 302 Location /a/{user}?face={face}
  U->>F: GET /a/{user}?face={face}
  F->>B: attach(face) via TTYD
